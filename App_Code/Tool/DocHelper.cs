...
namespace Tool
{
    public class DocHelper
    {
        ...
        /// <returns></returns>
        public static BaseResult SetWord(string templateUrlName, string targetUrl, Dictionary<string, string> wordList, DataSet data = null, Dictionary<string, string> picList = null)
        {
            ...
            try
            {
                var templateUrl = HttpContext.Current.Server.MapPath($"~/{templateUrlName}");
                ...
                if (picList != null)
                    ...
                    {
                        if (File.Exists(item.Value)) fillContent.Images.Add(new ImageContent(item.Key, File.ReadAllBytes(item.Value)));
                        else fillContent.Images.Add(new ImageContent(item.Key, File.ReadAllBytes(HttpContext.Current.Server.MapPath("~/default.jpg"))).Hide());
                    }
                if (data != null)
                    foreach (DataTable _table in data.Tables)
                    {
                        if (_table.Rows.Count > 0)
                        {
                            ...
                            {
                                var tableFields = new List<IContentItem>();
                                foreach (DataColumn _collumn in _table.Columns)
                                {
                                    if (_collumn.ColumnName.Contains("_Pic"))
                                    {
                                        if (File.Exists(_row[_collumn.ColumnName].ToString()))
                                        {
                                            tableFields.Add(new ImageContent($"@{_collumn.ColumnName}", File.ReadAllBytes(_row[_collumn.ColumnName].ToString())));
                                        }
                                    }
                                    else
                                    {
                                        tableFields.Add(new FieldContent($"@{_collumn.ColumnName}", ReplaceLowOrderASCIICharacters(_row[_collumn.ColumnName].ToString()).Replace("\n", "\r\n")));
                                    }
                                }
                                tableContent.AddRow(tableFields.ToArray());
                            }
